...
# Step 2: Re-apply with paragraph formatting (create doesn't support spacing flags)
gog docs write <docId> --file document.md --markdown --replace \
  --line-spacing 115 --space-below 8pt
```
...
Use `--check-orphans` to block replacement when open comment quotes would disappear.
...
| `line_spacing` | 115 | `gog docs write --line-spacing N` |
| `space_below` | 8pt | `gog docs write --space-below Npt` |
...
Repeat for each table, adjusting `--table-index`.
...
gog docs cell-style <docId> --table-index 1 \
  ...
  --background-color "#E5E5E5" --bold
```

### Step 4: Compact row heights

```bash
gog docs table-row style <docId> --table 1 --min-height 0pt
```
...
4. **Import**: `gog docs write <docId> --file doc.md --markdown --replace --pageless --line-spacing 115 --space-below 8pt --check-orphans`
5. **Format tables**: apply padding, header styling, and compact heights (see above)
6. **Insert images**: add any local images via `insert-image` (see above)
