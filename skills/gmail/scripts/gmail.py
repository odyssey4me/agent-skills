...
def get_oauth_client_config(service: str) -> dict[str, Any]:
    ...
    raise AuthenticationError(
        ...
        f"Run: python gmail.py auth setup --client-id YOUR_ID --client-secret YOUR_SECRET"
    )
...
def handle_api_error(error: HttpError) -> None:
    ...
    if status_code == 403 and "insufficient" in message.lower():
        scope_help = (
            ...
            "Or for custom OAuth, see: skills/gmail/references/oauth-setup.md\n"
        )
    ...
    if result["authenticated"]:
        ...
        if scopes:
            ...
            if not all_granted:
                ...
                print()
                print("   Or see: skills/gmail/references/oauth-setup.md")
        return 0
    else:
        ...
        if gcloud_available:
            ...
            print(
                "       python scripts/gmail.py auth setup --client-id YOUR_ID --client-secret YOUR_SECRET"
            )
            print("    3. Run any command to trigger OAuth flow")

        print()
        print("For detailed setup instructions, see: skills/gmail/references/oauth-setup.md")
        return 1
